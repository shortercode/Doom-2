...
const ATTRIBUTES = {
	child(child)
	{
		if (isObject(child) || isType(child, "string"))
			create(child, this);
		else if (isInstance(child, HTMLElement))
			this.appendChild(child);
		else
			throw new TypeError(`Invalid child "${child}"`);
	},
	children(children)
	{
	...
	data(attributes)
	{
		...
		for (let attributeName in attributes)
		{
			this.setAttribute("data-" + attributeName, attributes[attributeName]);
		}
	}
	// template(str) {
...
function create(options, parent, ...parameters)
{
	let element;
	...
	else
	{
		...
		// now onto creating the element
		// alternate element creation method for a shadow root
		if (tag === "shadow")
		{
			if (!parent)
				throw new Error('Shadow roots must have a parent');

			element = parent.attachShadow(
			{
				mode: "open"
			});
			// remove the parent reference, don't need to append the shadow root to the parent
			parent = null;
		}
		else
		{
			// search for an alloy with that tag
			let alloy = getAlloyDefinition(tag);
			// if we have an alloy definition use that, otherwise use createElement
			element = alloy ? new alloy(...parameters) : document.createElement(tag);
		}
