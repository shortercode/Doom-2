...
const ATTRIBUTES = {
  children(children) {
    ...
    for (let i = 0, l = children.length; i < l; i++) {
        ...
        let isTagName = isType(child, "string") && TAG_REGEX.test(child);
        let isCreatePattern = isObject(child);
        let isElement = isInstance(child, HTMLElement);
        
        if (isTagName || isCreatePattern) {
            child = create(child);
            isElement = isInstance(child, HTMLElement);
        }
        
        if (!isElement)
            throw new TypeError(`${child} is not a valid child of ${this}`);
    }
